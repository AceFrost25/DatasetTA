...
def load_images(path, ID):
    ...
    img_file = path + 'ir1/%s.png' % (ID*LABEL_INTERVAL)
...
def load_pcds(path, downsample = True, interval = 1):
    ...
    for Filename in xrange(len(glob.glob1(path+"ir1","*.png"))/interval):
        img_file = path + 'ir1/%s.png' % (Filename*interval)
...
if __name__ == "__main__":
    ...
    for path in folders:
        ...
        n_pcds = int(len(glob.glob1(path+"ir1","*.png"))/LABEL_INTERVAL)
        ...
        num_annotations = int(len(glob.glob1(path+"ir1","*.png"))/LABEL_INTERVAL)

        for point_id in range(num_annotations):
             Ts.append(pose_graph.nodes[point_id].pose)
        Ts = np.array(Ts)
        filename = path + 'transforms2.npy'
        np.save(filename, Ts)
